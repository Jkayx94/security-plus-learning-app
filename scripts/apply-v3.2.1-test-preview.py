...
boss_helpers = r'''function bossArenaMarkup(s:Session,phase:'question'|'hit'|'critical'|'miss'|'result'='question'){
 ...
 return `<section class="bossArena ${stateClass}" aria-label="Boss battle status"><div class="bossVisual ${s.bossType} ${stateClass}" aria-hidden="true"><span class="bossCore">◆</span><b>${bossName}</b></div>${bossConsole(s,status)}<section class="bossHud"><div class="bossTitle"><b>${esc(s.bossType)} boss</b><span>${s.bossThreshold}% required</span></div><div class="bossHealth"><i style="width:${hp}%"></i></div><small>Boss HP ${hp}% · ${correct}/${total} successful hits</small>${finishReady?'<p class="finishPrompt">FINISH MODE ARMED — the final strike unlocks after the last question.</p>':''}</section></section>`;
}
...
@keyframes cosmeticRise{from{opacity:.75;transform:translateY(4px)}to{opacity:1;transform:none}}@keyframes bossHit{25%{transform:translateX(-7px);filter:brightness(1.7)}60%{transform:translateX(5px)}}@keyframes bossCritical{20%{transform:scale(1.06);filter:brightness(2)}45%{transform:translateX(-10px) rotate(-1deg)}70%{transform:translateX(7px)}}@keyframes bossBlock{50%{transform:scale(.98);filter:saturate(.4)}}@keyframes finishPulse{50%{filter:brightness(1.35);transform:scale(1.01)}}@keyframes bossDefeat{to{opacity:.35;transform:scale(.88) translateY(10px);filter:grayscale(1)}}
